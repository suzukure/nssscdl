...
export PR_JSON PR_FINAL_JSON BASE_JSON RELATION_JSON ISSUE_JSON REVIEWS_JSON
...
gh() {
  ...
  case "$*" in
    'api repos/owner/repo/pulls/37')
      [ "${GH_FAIL:-}" != pr ] || return 1
      if [ -n "${PR_FINAL_JSON:-}" ] && [ "$(grep -c 'api repos/owner/repo/pulls/37' "$GH_LOG")" -gt 1 ]; then
        printf '%s\n' "$PR_FINAL_JSON"
      else
        printf '%s\n' "$PR_JSON"
      fi ;;
...
assert_decision() {
  ...
  if [ "$action" = ignore ] && [ "$code" = stale_head ] && [ "$name" != final-stale ] \
...
if grep -q 'pr view\|pulls/37/reviews' "$GH_LOG"; then
  echo 'Initial stale dispatch fetched relation or review state.' >&2
  exit 1
fi

PR_JSON="$(jq -c '.user.login="human"' <<< "$PR_JSON")"
...
assert_decision newer-change-request proceed ready \
  "$(jq -c '.round=2' <<< "$payload")" "${gate[@]}"
PR_FINAL_JSON="$(jq -c '.updated_at="2026-01-10T00:00:00Z" | .mergeable_state="dirty" |
  .labels += [{name:"unrelated-label"}]' <<< "$PR_JSON")"
assert_decision volatile-fields proceed ready "$(jq -c '.round=2' <<< "$payload")" "${gate[@]}"
PR_FINAL_JSON="$(jq -c '.draft=true' <<< "$PR_JSON")"
assert_decision state-changed human_required state_changed "$(jq -c '.round=2' <<< "$payload")" "${gate[@]}"
PR_FINAL_JSON="$(jq -c --arg sha "$old_sha" '.head.sha=$sha' <<< "$PR_JSON")"
assert_decision final-stale ignore stale_head "$(jq -c '.round=2' <<< "$payload")" "${gate[@]}"
PR_FINAL_JSON=''
