...
canonical_pr_facts() { jq -cse --arg repo "$repo" --argjson number "$pr_number" '
  def sha: type == "string" and test("^[0-9a-f]{40}$");
...
' 2>/dev/null; }
pr_facts="$(canonical_pr_facts <<< "$pr")" || human invalid_pr 'Current PR metadata is inconsistent.'
...
relation_head="$(jq -rse 'if length == 1 and (.[0].headRefOid | type) == "string"
  and (.[0].headRefOid | test("^[0-9a-f]{40}$")) then .[0].headRefOid
  else error("relation head") end' <<< "$relation" 2>/dev/null)" \
  || human invalid_relation 'PR closing Issue relation HEAD is invalid.'
[ "$relation_head" = "$current_head" ] \
  || ignore stale_head 'PR HEAD changed while fetching closing Issue relation.'
issue_number="$(jq -rse --arg repo "$repo" --arg head "$current_head" \
...
final_head="$(jq -rse 'if length == 1 and (.[0].head.sha | type) == "string"
  and (.[0].head.sha | test("^[0-9a-f]{40}$")) then .[0].head.sha
  else error("head") end' <<< "$final_pr" 2>/dev/null)" \
  || human invalid_pr 'Final PR HEAD is invalid.'
if [ "$final_head" != "$validated_sha" ]; then
  ignore stale_head 'PR HEAD changed while evaluating the dispatch.'
fi
final_facts="$(canonical_pr_facts <<< "$final_pr")" \
