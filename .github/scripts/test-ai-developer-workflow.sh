...
grep -Fq 'expected_preflight_marker="Service-local hardening preflight verified AF_UNIX/AF_INET and protected UNIX socket boundary."' "$developer_step"
grep -Fq -- '--unit="$unit"' "$developer_step"
...
grep -Fq 'marker_rc=$?' "$developer_step"
grep -Fq 'Service-local hardening preflight success marker unavailable from unit journal.' "$developer_step"
grep -Fq 'exit 51' "$developer_step"
grep -Fq 'printf "%s\n" "$preflight_marker"' "$developer_step"
