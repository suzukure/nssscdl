...
grep -Fq '      - name: Configure probe paths' "$probe_job"
grep -Fq '            echo "PROBE_MARKER=$RUNNER_TEMP/background-cancel-probe.marker"' "$probe_job"
grep -Fq '            echo "PROBE_PID=$RUNNER_TEMP/background-cancel-probe.pid"' "$probe_job"
grep -Fq '            echo "PROBE_DONE=$RUNNER_TEMP/background-cancel-probe.done"' "$probe_job"
grep -Fq '          } >> "$GITHUB_ENV"' "$probe_job"
