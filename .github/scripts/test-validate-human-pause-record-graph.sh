...
assert_valid single-root-linear-chain "$(graph "$normalization_3" "$root_1" "$replacement_2")"

root_4="$(record 201 -)"
replacement_5="$(record 202 201)"
root_6="$(record 301 -)"
assert_valid multiple-roots-and-historical-chain "$(graph "$replacement_5" "$root_6" "$root_4")"
