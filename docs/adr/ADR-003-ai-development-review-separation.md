...
- 要求変更・人間判断マーカー、または3回目のchange requestで関連IssueとPRの両方へ `human-review-required` を付け、Codexの自動修正と自動マージを止める。Codexの要求変更マーカーは単独行で完全一致する `[REQUIREMENTS_CHANGE_REQUIRED]` だけを有効とし、Codex実行後のIssue起点とClaude review follow-upで同じ規則を適用する。Issue起点ではdeveloper App token発行前にIssueと対応PRの両方を検査し、同期・差分・closing Issue取得に失敗した場合もfail-closedとする。通知用Discord Webhookが設定済みなら同時に通知する。
