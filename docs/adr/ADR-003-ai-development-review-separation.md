...
- 要求変更・人間判断マーカー、または3回目のchange requestで関連IssueとPRの両方へ `human-review-required` を付け、Codexの自動修正と自動マージを止める。Codexの要求変更マーカーは、CRLFを含むplain textの単独行で完全一致する `[REQUIREMENTS_CHANGE_REQUIRED]` だけを有効とする。backtick、字下げ、前後空白、説明文中の言及は有効にしない。Codex実行後のIssue起点とClaude review follow-upで同じtrusted helperを適用し、helperまたはそのbootstrapの失敗は「マーカーなし」と扱わずfail-closedとする。Claude reviewerの人間判断マーカーはstructured review summaryを対象にreviewer側で解釈するため、Codexのplain-text最終応答とは検出経路が異なる。Issue起点ではdeveloper App token発行前にIssueと対応PRの両方を検査し、同期・差分・closing Issue取得に失敗した場合もfail-closedとする。通知用Discord Webhookが設定済みなら同時に通知する。
