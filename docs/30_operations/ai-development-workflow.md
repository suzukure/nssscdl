...
`.github/scripts/create-human-pause.sh` は上記のtrusted listingとreconciliationを使うhuman pause遷移境界である。`create REPO ISSUE PR APP_ID REASON DETAIL` はactive pauseがなければ選択したConversationにroot `pause` recordを投稿し、REST comment IDを`pause_id`とする。IssueまたはPRの一方は`-`で省略できるが、少なくとも一方を指定する。PRがあればrecordはPR Conversationに置く。既存active pauseが同じreasonならラベルだけを再同期して`already_active`を返し、別reasonならfail-closedとする。`inspect REPO ISSUE PR APP_ID PAUSE_ID` は既存IDのactive / consumedを確認し、activeならラベルを再同期し、`already_active` / `already_consumed`を返す。resume拒否や既存pauseの再検出には`inspect`を使い、これらの経路では通知しない。新しいpause作成時は `human-review-required` を関連Issue / PRへ同期し、作成したrecordがtrusted listingで確認できてからのみ `.github/scripts/format-human-pause-notification.sh` のreason別日本語文を `.github/scripts/notify-human.sh` でbest-effort送信する。ラベル・record整合の失敗では通知せず停止し、Discord未設定・送信失敗ではGitHub上のpauseを維持する。自由文DETAILは判断内容の表示とrecord payloadに限り、workflow制御には使用しない。machine-only stateはrecord schemaと通知formatterのreason allowlistに含めない。このhelperのproduction workflowへの経路別移行は本段階では行わない。
