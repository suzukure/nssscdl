...
native Codex exec前には同じservice / `setpriv` contextで、UID/GID、supplementary groups empty、`NoNewPrivs=1`、全capability zero、`sudo -n true` の失敗、AF_UNIX socket作成成功、AF_INET socket作成成功をfail-closedに確認する。さらに固定13 pathについて、存在する場合はservice viewがsocket / mode 0000 / runner identityからR/W/X不可であることを確認する。その後 `/run` をread-only走査し、mask後もrunner identityからwrite可能なroot-owned UNIX socketが1件でも残れば、未知のrunner-image driftとしてnative Codex/model call前にfail-closedする。permission上traverse不能なpathとscan中に消滅したpathはworkloadから到達不能または通常のruntime raceとしてskipするが、それ以外のscan errorはfail-closedとする。socketへconnectは行わず、host側permissionも変更しない。このpreflightはtransient serviceのExecStart内で実行されるため `RuntimeMaxSec` の内側に含まれる。失敗時はdiagnosticをunit journalへ残し、exit codeを `39=sudo検査不能 / 40=sudo保持 / 41=UID不一致 / 42=GID不一致 / 43=supplementary groups残存 / 44=NoNewPrivs不成立 / 45=capability非zero / 46=AF_UNIX拒否 / 47=AF_INET拒否 / 48=固定socket mask不成立 / 49=残存writable root-owned UNIX socketまたはscan異常` として付与する。これらのcodeはnative Codex自身のexit codeと衝突し得るため、code単独で原因を確定せず、unit限定journalの `Service-local hardening preflight ...` diagnostic messageと併読して判定する。#368 Run `35505392927`、#371 / #372 / #374でblanket AF_UNIX denyがbubblewrapの `Failed to look up lo` の直接原因と確定し、#376 / Run `35507942245` でAF_UNIXを許可したproduction同型境界からactual Terra local tool成功、#377 / Run `35508896886` で13 socket mask・actual Terra local tool成功・host非破壊を同時に実証済みである。

Codexはこのhardening後かつservice cgroup内でvalidated native binaryを直接実行する。service commandは `/usr/bin/env -i` から開始し、`HOME` / `USER` / `LOGNAME` / `PATH` / `RUNNER_TEMP` / `GITHUB_WORKSPACE` / `CODEX_HOME` / `CODEX_FINAL` / `CODEX_PROMPT_FILE` / `CODEX_MODEL` / `CODEX_NATIVE` / `CODEX_PACKAGE_ROOT` / `CODEX_INTERNAL_ORIGINATOR_OVERRIDE` / `PROTECTED_UNIX_SOCKET_PATHS` だけを明示allowlistとして渡す。最後の値は上記13件の非機密な固定path listで、同一service preflightが `InaccessiblePaths` の実効性を検証するためだけに使用する。API key、GitHub App token、setup stepのその他environmentは継承しない。npm launcher parityとして `CODEX_MANAGED_PACKAGE_ROOT=<validated package root>`、`CODEX_MANAGED_BY_NPM=1` をchild launcher内で付与し、Bun / pnpm / Vite+ markerはunsetする。pin済みAction sourceではResponses API endpointは追加environmentではなく `CODEX_HOME/config.toml` のlocalhost providerで渡されるため、serviceはこのallowlistだけでproxyを利用する。actual Codex + localhost request pathは#363 / Run #137でservice-local hardening下でも成立済みである。
