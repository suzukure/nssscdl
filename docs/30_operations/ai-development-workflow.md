...
#320 main反映後の #307 runtime再検証手順は #320 Issue本文を正本とする。運用文書側では手順を重複定義しない。実行前に#307本文へcurrent再開条件を同期し、branch / PR / unexpected repository write不存在とcurrent implementation contractを確認する。通常 `/codex develop` は人間判断で停止解除後に1回だけ投入し、allowlist環境下のlocalhost Responses proxy経由model call、`:workspace` の実効permission境界、service-local preflight / setpriv / systemd cgroup収束、およびhost service socket / DNS非破壊を非機密証跡で確認する。いずれかを確認できない場合はautomatic retryせず#328へ戻る。
