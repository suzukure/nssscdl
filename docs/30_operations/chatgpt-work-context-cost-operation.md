...
既に多数のProjectチャットが蓄積している場合は、個々のチャットを恒久保存対象として選別するのではなく、開発継続に必要なchat-only情報が残っていないかを先に監査してGitHubへ収束させる。

- 未決で今後必要なchat-only情報は対応するopen Issueへ移す。
- 確定済みだがmain未反映のchat-only情報は、既存正本文書へ反映するIssue / PRへ移す。
- main / Issue / PR / Actions証跡に既に存在する履歴、解消済み障害、過去の試行錯誤、重複説明は移行しない。
- 監査のためだけの恒久的な`chat_archive.md`やhandoff台帳をrepositoryへ追加しない。監査結果を継続判断に残す必要がある場合は、対応するIssueへ記録する。
- 監査完了条件は、GitHub main + open Issues + open PRsから現在の開発継続状態を再構成でき、過去Projectチャットを開発上の情報源として保持する必要がないことである。
- ChatGPT側でarchive/deleteを行う場合も、GitHubへの収束を先に完了させる。archive/delete自体を仕様・設計・実行結果の正本変更として扱わない。
