...
| 要求ベースライン | `docs/00_requirements/` v1.4 |
| ベースライン確認日 | 2026-08-29 |
| Git基準 | `main` commit `347bf72645c55f3fb791c72d84e4704db59f5567` 時点の要求仕様 |
...
| 優先度 | 意味 | 主な対象 |
|---|---|---|
| P0 | 失敗時に予約整合性、認証、PII、復旧性へ重大影響 | 予約競合、キャンセル境界、未来枠整合性、分類、認証、削除、RPO/Backup |
...
| Severity | 定義例 |
|---|---|
| Critical | 二重予約、他人予約の露出、認証回避、PII重大漏えい、復旧不能 |
| Major | 主要業務が実行不能、誤った取消・分類、期限判定誤り、通知失敗で業務状態がRollback |
