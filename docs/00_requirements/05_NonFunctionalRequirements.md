...
- AC-913-005 外部Providerの料金プラン、課金、利用量、Quota等の管理情報を確認するアクセスを必要最小限とし、通常の利用者操作または業務処理ごとに問い合わせず、無料枠維持等の確認のみを目的とした継続的・高頻度なPollingを行わない。
- AC-913-006 外部Providerの料金プラン変更機能を通常のアプリケーション実行経路から分離し、アプリケーション処理によって自動的または暗黙に有料Planへ変更できないようにする。

## REQ-914 障害・エラー時利用者表示
計画保守、障害、部分障害、操作失敗時に、利用者へ安全かつ理解可能な状態と必要な対応案内を表示する。
