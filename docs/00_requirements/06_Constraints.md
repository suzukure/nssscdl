...
| OI-TM-052 | Closed | 生徒一括予約は同一生徒・同一月、月間標準回数Nを1回の操作上限として一括Preview／一括確定する。選択対象外の既存未開始Reservationの区分変化は最終確定前に表示する。全体Reject、Preview／Confirm時の再検証および追加予約の公平性制御を初期リリースへ追加しない判断を含む。 |
